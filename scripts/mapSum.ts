...
`
{
    ...
    class MapSum {
        valDict: Map<string, number>;

        constructor() {
            this.valDict = new Map<string, number>();
        }

        insert(key: string, val: number): void {
            this.valDict.set(key, val);
        }

        sum(prefix: string): number {
            let count = 0;
            [...this.valDict.entries()].forEach(([key, val]) => {
                if (key.slice(0,prefix.length) === prefix){
                    // console.log(`adding ${key} with val ${val}`);
                    count += val;    
            }});
            return count;
        }
